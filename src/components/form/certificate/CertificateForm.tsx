...
const CertificateForm: React.FC<CertificateFormProps> = ({
  ...
  const onSubmit = async (values: CertificateInput) => {
    if (hasResident) {
      if (await validateAndSubmit()) setStep(4);
      return;
    }
    nextStep();
    scrollToForm();
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <SelectContent>
                  {Object.entries(CertificateType).map(([key, value]) => (
                    <SelectItem key={key} value={value}>
                      {getCertificateTypeLabel(
                        key as keyof typeof CertificateType,
                      )}
                    </SelectItem>
                  ))}
                </SelectContent>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                Purpose <span className="font-bold text-red-600">*</span>
              </FormLabel>
              <FormControl>
                <Input
                  {...field}
        ...
        {[
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  Request of <span className="font-bold text-red-600">*</span>
                </FormLabel>
                <FormControl>
                  <Input
                    {...field}
